...
rain_ras <- raster("../data/raw/spatial/rfgrid_mm_oahu_ann.txt", crs = std_proj)
...
Basically, all of the earlier pieces put together
...
# run function to convert points for 3d plotting with rgl::points3d or rgl::lines3d
## Sites
sites <- custom_add_points(
            elev_ras,
            samps$latitude_deg,
            samps$longitude_deg,
            raise_agl = 10,
...
## Rivers
rivers <- st_coordinates(waimea_river) %>% as.data.frame()

river_list <- unique(rivers$L1)
river_coords <-list()

river_coords <- lapply(river_list, function(x){
                    a_river <- rivers[rivers$L1 == i,]
                    custom_add_points(elev_ras,
                                        a_river[,2],
                                        a_river[,1],
                                        raise_agl = 10,
                                        zscale = zscale,
...
test <-  custom_add_points(elev_ras,
                                        rivers[,2],
                                        rivers[,1],
                                        raise_agl = 10,
                                        zscale = zscale,
...
# add points
rgl::points3d(sites$x, sites$y, sites$z, 
                   color = sites$col, alpha = sites$alpha, size = sites$size)



rgl::lines3d(test$x, test$y, test$z, color = "blue", lwd = 1, alpha = 1)


for(a in seq_along(river_coords)) {
  river <- river_coords[[a]]
  rgl::lines3d(river$x, river$y, river$z, color = "blue", lwd = 1, alpha = 1)
}

render_snapshot()
render_scalebar(limits=c(0, 1, 5),label_unit = "km",position = "N", y=10, scale_length =0.25, color_first = "darkgray", radius = 1)
```

# Add Lines
I want to add the waimea river as a line floating above the map.
