...
# sample metadata
raw_samples  <- read.table("../data/raw/unique_waimea_hiseq_samples_emp.tsv", sep = "\t", header = T)

# library prep metadata
prep_16S <- read.table("../data/raw/combined_hiseqs_bioblitz_mapping - 16S_prep_info.tsv", sep = "\t", header = T)
prep_ITS <- read.table("../data/raw/combined_hiseqs_bioblitz_mapping - ITS_prep_info.tsv", sep = "\t", header = T)
...
# filter by project
samples <- raw_samples %>% filter(project_name %in% c("Keck","JoshMS"))
...
samples %>% select(habitat, sample_type) %>% group_by(habitat, sample_type) %>% summarise(n())
# sample counts by site
samples %>% select(site_name, sample_type) %>% group_by(site_name) %>% summarise(n())
```
...
troph_by_habitat <- samples %>% select(habitat, trophic) %>% group_by(habitat, trophic) %>% summarise(count = n())

# barplot of counts by habitat, stacked trophic levels
ggplot(troph_by_habitat, aes(y = count, x = habitat, fill = trophic )) +
...
```{r}
## normal barplots
troph_data <- samples %>% group_by(habitat, common_name, trophic) %>% summarise(value = n())
troph_data$group <- troph_data$habitat
troph_data$key <- paste(troph_data$habitat, troph_data$common_name)
troph_data <- troph_data[!is.na(troph_data$habitat),]

## unique ids for each bar
ids <- troph_data %>% group_by(key, habitat) %>% summarise(tot = sum(value))
ids <- arrange(ids,habitat,tot)
ids$id <- 1:nrow(ids)

troph_data <- left_join(troph_data, ids)
troph_data <- arrange(troph_data, habitat, tot)
...
p<- ggplot(data=habidata, aes(x=reorder(common_name,id), y = value, fill=trophic)) +
...
p <-
  ggplot(data = troph_data, aes(x = factor(id), y = value, fill = trophic)) +
  geom_bar(stat = "identity") +
  labs(fill = "Trophic Level") +
  scale_fill_viridis_d(option = "viridis") +
...
```{r}
library(plotly)
# reformat data into hierarchy for plotting sunburst
# see https://stackoverflow.com/questions/57395424/how-to-format-data-for-plotly-sunburst-diagram
