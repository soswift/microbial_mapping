...
stopifnot(getwd() == "/home/sean/Documents/Bioinformatics/Projects/Waimea/microbial_mapping/main")
...
# sample metadata
raw_samples  <- fread("../data/raw/sample/unique_waimea_hiseq_samples_emp.tsv", sep = "\t", header = T)

# library prep metadata
prep_16S <- fread("../data/raw/sample/combined_hiseqs_bioblitz_mapping - 16S_prep_info.tsv", sep = "\t", header = T)
prep_ITS <- fread("../data/raw/sample/combined_hiseqs_bioblitz_mapping - ITS_prep_info.tsv", sep = "\t", header = T)

head(raw_samples)
head(prep_16S)


all_16S <-left_join(prep_16S, raw_samples)
all_ITS <- left_join(prep_ITS, raw_samples)

all_16SandITS <- rbind(all_16S, all_ITS)

n_distinct(raw_samples$sample_name)
n_distinct(prep_16S$sample_name)


write.csv(all_16S, "combined_prep_sample_metadata_16S.csv", row.names = F)

write.csv(all_ITS, "combined_prep_sample_metadata_ITS.csv", row.names = F)

write.csv(all_16SandITS, "combined_prep_sample_metadata_16SandITS.csv", row.names = F)
...
- *envo_biom_1e* = Categorical variable following earth microbiome project.
...
```{r}
# projects
unique(raw_samples$project_name)

# filter by project
samples <- raw_samples[project_name %in% c("Keck","JoshMS"),]
...
Summarize the sample types in each habitat and by each site to make sure verythign looks clean.
...
ggplot(troph_by_habitat, aes(y = N, x = habitat, fill = trophic )) +
...
p<- ggplot(data=habidata, aes(x=reorder(common_name,id), y = N, fill=trophic)) +
...
print(p)
ggsave(file = paste("../outputs/figures/_", habitats[a],"_barplot.pdf",sep = ""),
       plot = p, device = "pdf", width = 10, height = 10, dpi = 400)
...
```{r}
# TODO improve text alignment/remove empty bars (excess IDs?) only if we are going to use these plots
...
geom_text(data=label_data, aes(x=factor(id), y=tot+10, label=common_name, hjust=hjust), color="black", fontface="bold",alpha=0.6, size=6, angle= label_data$angle, inherit.aes = FALSE )   
#
p
# 
ggsave("../outputs/figures/habitat_sample_type_circleplot.pdf",plot = p, device = "pdf", width = 15, height = 10, dpi = 400)
...
```{r}
# TODO re-write as.sunburstDF to better understand
# TODO write snippet to assign uniform colors to middle circle


library(plotly)
source("../src/plot_functions.R")

sunburstDF <- as.sunburstDF(troph_data[c(1,3,2,4)], valueCol = "value")
...
orca(fig, file = "../outputs/figures/summary_sunburst.pdf",format ="pdf") 
```

## Data subsets
Checking up on the total counts of drosophila and other invertebrates for Mel.
These squencing files will be copied over so they can be analyzed independently.

Same for Matt and mosquitoes (specificall aedes albopictus)

```{r}
# mosquito
samples[grepl("albopictus",metadata)] %>% write.csv("~/albopictus_hiseq.csv")

# Drosophila and inverts
inverts <-
  c(
  "DrosophilaLab",
  "DrosophilaBaited",
...
samples[sample_type %in% inverts] %>% fwrite("../data/processed/waimea_hiseq_inverts.csv")
```
