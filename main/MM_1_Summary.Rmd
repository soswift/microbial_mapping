...
# Document Summary
This document is a guide to the different metadata categories, sample numbers, sampling sites, etc. that are included in the Waimea dataset. 

The goal is to give an introduction to the data collection and project structure.
...
- *sample_name* = Unique integer coresponding to a sample taken from the field. Exception: controls duplicated across sequencing runs were given unique IDs for the same sample. See 'metadata' column for more information on these controls.
...
- *project_name* = For microbial mapping, the project should be "Keck". A few unrelated samples are included in the sequencing run that are not included in the analysis. 
